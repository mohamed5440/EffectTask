...
import { getStatusLabel, getStatusColors } from "../../lib/formatters";
...
export const DurationBadge: React.FC<DurationBadgeProps> = ({ duration, className }) => {
  ...
  const formatted = typeof duration === 'number' ? `${duration} أيام` : duration;
  const label = formatted === "1" ? "يوم واحد" : (formatted === "2" ? "يومان" : formatted);
